        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form method="post" onsubmit="return price22(this)" action="/manager/invoice/pay/add/{{$invoice_user["id"]}}/{{$user["id"]}}" >
                @csrf
                ...
                @if(\App\Http\Controllers\managerController::managerAccessLevel(70))
                  <td style="display: none;" class="d-block-s">
                    @if($invoice_pay['active_pay'] != 3)
                      <a href="/manager/invoice/pay/active/{{$invoice_pay["ID"]}}" data-toggle="tooltip" data-original-title="@if($invoice_pay['active_pay'] == 1){{'رد'}}@else{{'تایید'}}@endif" class="btn btn-@if($invoice_pay['active_pay'] == 1){{'warning'}}@else{{'success'}}@endif">
